...
export function register(pi: ExtensionAPI): void {
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            summarize: Type.Optional(
                Type.Boolean({
                    description: "Summarize scraped content using the Pi agent",
                }),
            ),
        }),
        async execute(_toolCallId, params, signal, _onUpdate, ctx) {
            ...
            const result = await execWebclaw(args, signal);

            // Summarize if requested
            if (params.summarize && result.content[0] && ctx?.model) {
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            summarize: Type.Optional(
                Type.Boolean({
                    description: "Summarize extracted content using the Pi agent",
                }),
            ),
        }),
        async execute(_toolCallId, params, signal, _onUpdate, ctx) {
