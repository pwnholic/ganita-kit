...
:root{--bg:#18181e;--bg-card:#1e1e24;--bg-elevated:#252530;--bg-hover:#2b2b37;--fg:#e0e0e0;--fg-muted:#909098;--fg-dim:#606068;--accent:#8abeb7;--accent-hover:#9dcec7;--accent-muted:rgba(138,190,183,0.15);--border:#2a2a34;--border-muted:#353540;--btn-primary:#8abeb7;--btn-primary-hover:#9dcec7;--btn-primary-fg:#18181e;--btn-secondary:#252530;--btn-secondary-hover:#2b2b37;--success:#b5bd68;--warning:#f0c674;--font:system-ui,-apple-system,sans-serif;--radius:10px;--radius-sm:6px}
@media(prefers-color-scheme:light){:root{--bg:#f5f5f7;--bg-card:#fff;--bg-elevated:#eeeef0;--bg-hover:#e4e4e8;--fg:#1a1a1e;--fg-muted:#6c6c74;--fg-dim:#9a9aa2;--accent:#5f8787;--accent-hover:#4a7272;--accent-muted:rgba(95,135,135,0.12);--border:#dcdce0;--border-muted:#c8c8d0;--btn-primary:#5f8787;--btn-primary-hover:#4a7272;--btn-primary-fg:#fff;--btn-secondary:#e4e4e8;--btn-secondary-hover:#d4d4d8;--success:#4d7c0f;--warning:#b45309}}
body{font-family:var(--font);background:var(--bg);color:var(--fg);line-height:1.5;min-height:100dvh;padding-bottom:72px;margin:0}
.timer-badge{position:fixed;top:20px;right:24px;z-index:50;font-size:12px;font-weight:600;font-variant-numeric:tabular-nums;padding:5px 14px;border-radius:999px;background:var(--bg-elevated);color:var(--fg-muted);border:1px solid var(--border);cursor:pointer;user-select:none;opacity:0.5;transition:opacity 0.3s}
.timer-badge:hover{opacity:1}.timer-badge.warn{background:rgba(240,198,116,0.15);color:#f0c674;opacity:1}.timer-badge.urgent{background:rgba(204,102,102,0.15);color:#cc6666;opacity:1}
.hero{padding:40px 24px 24px;text-align:center}.hero-kicker{font-size:11px;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;color:var(--fg-muted);margin-bottom:8px}
.hero-title{font-size:28px;font-weight:700;color:var(--fg);margin-bottom:4px}.hero-desc{font-size:14px;color:var(--fg-muted);margin-bottom:20px}
.hero-meta{display:flex;align-items:center;justify-content:center;gap:12px;font-size:13px;color:var(--fg-muted)}
.hero-meta-sep{width:4px;height:4px;border-radius:50%;background:var(--fg-dim)}
.provider-buttons{display:flex;gap:6px}
.provider-btn{padding:4px 12px;border-radius:999px;border:1px solid var(--border);background:var(--bg-card);color:var(--fg-muted);font-size:12px;cursor:pointer;transition:all 0.2s}
.provider-btn:hover{border-color:var(--accent);color:var(--accent)}.provider-btn.is-default{background:var(--accent-muted);color:var(--accent);border-color:var(--accent)}
#result-cards{max-width:720px;margin:0 auto 24px;padding:0 16px}
.add-search{display:flex;align-items:center;gap:8px;max-width:720px;margin:0 auto 24px;padding:0 16px}
.add-search-icon{font-size:20px;color:var(--fg-dim);flex-shrink:0}
.add-search input{flex:1;padding:12px 16px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--bg-card);color:var(--fg);font-size:14px;outline:none;transition:border-color 0.2s}
.add-search input:focus{border-color:var(--accent)}
.summary-panel{max-width:720px;margin:0 auto 24px;padding:0 16px;display:block}.summary-panel.hidden{display:none}
.summary-header{padding:16px 0}.summary-title{font-size:18px;font-weight:600;margin-bottom:4px}
.summary-input{width:100%;min-height:200px;padding:16px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--bg-card);color:var(--fg);font-size:14px;font-family:var(--font);resize:vertical;outline:none;transition:border-color 0.2s}
.summary-input:focus{border-color:var(--accent)}
.summary-generating{padding:40px 0;text-align:center}
.summary-actions{display:flex;gap:8px;justify-content:flex-end;padding:12px 0}
.btn{padding:10px 20px;border:none;border-radius:var(--radius-sm);font-size:14px;font-weight:500;cursor:pointer;transition:all 0.2s}
.btn:disabled{opacity:0.4;cursor:not-allowed}
.btn-submit{background:var(--btn-primary);color:var(--btn-primary-fg)}.btn-submit:hover:not(:disabled){background:var(--btn-primary-hover)}
.btn-secondary{background:var(--btn-secondary);color:var(--fg)}.btn-secondary:hover:not(:disabled){background:var(--btn-secondary-hover)}
.action-bar{position:fixed;bottom:0;left:0;right:0;display:flex;justify-content:flex-end;padding:12px 24px;background:var(--bg);border-top:1px solid var(--border);z-index:40}
.success-overlay,.expired-overlay{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(24,24,30,0.92);z-index:100;flex-direction:column;gap:12px;text-align:center}
.success-overlay.hidden,.expired-overlay.hidden{display:none}
.success-icon{font-size:36px;font-weight:700;color:var(--success);border:3px solid var(--success);width:64px;height:64px;border-radius:50%;display:flex;align-items:center;justify-content:center}
.expired-icon{font-size:36px;color:var(--warning);border:3px solid var(--warning);width:64px;height:64px;border-radius:50%;display:flex;align-items:center;justify-content:center}
.error-banner{position:fixed;top:0;left:0;right:0;padding:12px 24px;background:rgba(204,102,102,0.15);color:#cc6666;font-size:14px;z-index:200}
.result-card{background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:16px;margin-bottom:12px;cursor:pointer;transition:all 0.2s}
.result-card:hover{background:var(--bg-hover)}.result-card.selected{border-color:var(--accent);background:var(--accent-muted)}
.result-card-header{display:flex;align-items:flex-start;gap:12px}.result-card-check{width:20px;height:20px;border:2px solid var(--border-muted);border-radius:4px;flex-shrink:0;margin-top:2px;transition:all 0.2s}
.result-card.selected .result-card-check{background:var(--accent);border-color:var(--accent)}.result-card-check.checked::after{content:"\\2713";display:flex;align-items:center;justify-content:center;color:var(--btn-primary-fg);font-size:12px;font-weight:700}
.result-card-query{font-size:14px;font-weight:600;color:var(--fg);margin-bottom:4px}
.result-card-provider{font-size:11px;color:var(--fg-dim);margin-bottom:8px}
.result-card-answer{font-size:13px;color:var(--fg-muted);line-height:1.6;margin-bottom:8px}
.result-card-urls{font-size:12px}.result-card-url{color:var(--accent);text-decoration:none;display:block;padding:2px 0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.result-card-url:hover{text-decoration:underline}.result-card-error{font-size:13px;color:#cc6666}
