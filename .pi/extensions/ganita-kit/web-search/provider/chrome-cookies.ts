/**
 * Chrome/Chromium cookie extraction for Google services.
 *
 * Reads cookies from installed Chromium-based browsers (Chrome, Arc, Helium,
 * Chromium) on macOS and Linux. Uses OS keychain for decryption key.
 * Required for zero-config Gemini Web access.
 */
...
/**
 * Extract Google cookies from installed Chromium-based browsers.
 * Tries each browser in order (Helium → Chrome → Arc on macOS).
 * @param options - Optional profile name and required cookies filter
 * @returns Cookie map + warnings, or null if no browser found
 */
export async function getGoogleCookies(options?: {
    ...
    const profile = options?.profile ?? getChromeProfile();
    ...
    for (const config of configs) {
        ...
        try {
            ...
            for (const row of rows) {
                ...
                let value =
                    typeof rawValue === "string" && String(rawValue).length > 0
                        ? String(rawValue)
                        : null;
                if (!value) {
...
async function importSqlite(): Promise<any> {
    ...
    process.emitWarning = ((warning: string | Error, ...args: unknown[]) => {
        ...
        return (orig as (...args: unknown[]) => void)(warning, ...args);
...
function copySidecar(srcDb: string, targetDb: string, suffix: string): void {
    ...
    try {
        ...
        // Sidecar file is optional — ignore copy errors
    }
}
