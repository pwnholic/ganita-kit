...
import { register } from "../commands/fastedit.js";
...
describe("fast_read", () => {
	it("calls fastedit read with file path", async () => {
		const { pi, capture } = createMockPi(
			new Map([["fastedit", { stdout: "fn foo() {}" }]]),
		);
		register(pi as never);
...
describe("fast_edit", () => {
	it("uses default backend=mlx and model_path when no params given", async () => {
		const { pi, capture } = createMockPi(
			new Map([["fastedit", { stdout: "edited" }]]),
		);
		register(pi as never);
...
describe("fast_delete", () => {
	it("calls delete with file and symbol", async () => {
		const { pi, capture } = createMockPi(
			new Map([["fastedit", { stdout: "deleted" }]]),
		);
		register(pi as never);
		...
		await tool.execute(
			"d1",
			{ file: "src/app.ts", symbol: "oldHandler" },
			undefined,
		);
...
describe("fast_rename", () => {
	it("calls rename with file, old_name, new_name", async () => {
		const { pi, capture } = createMockPi(
			new Map([["fastedit", { stdout: "renamed" }]]),
		);
		register(pi as never);
...
describe("fast_diff", () => {
	it("calls diff with file", async () => {
		const { pi, capture } = createMockPi(
			new Map([["fastedit", { stdout: "diff output" }]]),
		);
		register(pi as never);
...
describe("fast_undo", () => {
	it("calls undo with file", async () => {
		const { pi, capture } = createMockPi(
			new Map([["fastedit", { stdout: "reverted" }]]),
		);
		register(pi as never);
...
describe("fast_pull", () => {
	it("calls pull without model", async () => {
		const { pi, capture } = createMockPi(
			new Map([["fastedit", { stdout: "downloaded" }]]),
		);
		register(pi as never);
