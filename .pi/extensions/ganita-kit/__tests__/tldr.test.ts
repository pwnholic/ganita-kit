...
describe("tldr_coupling", () => {
	it("passes path_a only for project-wide scan", async () => {
		...
		await capture.tools
			.get("tldr_coupling")!
			.execute("c12", { path_a: "src/" }, undefined);
...
describe("tldr_fix", () => {
	it("passes error output to fix diagnose", async () => {
		...
		await capture.tools
			...
			.execute(
				"c16",
				{ error_output: "TypeError: x is not iterable" },
				undefined,
			);
		const args = capture.execCalls[0]!.args;
...
describe("tldr infrastructure", () => {
	it("warm calls warm subcommand", async () => {
		const { pi, capture } = createMockPi(
			new Map([["tldr", { stdout: "warmed" }]]),
		);
		register(pi as never);

		await capture.tools
			.get("tldr_warm")!
			.execute("c17", { path: "src/" }, undefined);
	...
	it("daemon_status calls daemon status", async () => {
		...
		await capture.tools
			.get("tldr_daemon_status")!
			.execute("c20", {}, undefined);
		expect(capture.execCalls[0]!.args).toEqual([
			"daemon",
			"status",
			"-f",
...
describe("tldr error handling", () => {
	it("returns isError on non-zero exit", async () => {
		const { pi, capture } = createMockPi(
			new Map([["tldr", { stdout: "error", exitCode: 1 }]]),
		);
		register(pi as never);
