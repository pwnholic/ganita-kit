...
function deepMerge(
	...
	if (overrides.geminiApiKey !== undefined)
		result.geminiApiKey = overrides.geminiApiKey;
	if (overrides.chromeProfile !== undefined)
		result.chromeProfile = overrides.chromeProfile;

	if (overrides.search) {
		for (const key of Object.keys(overrides.search) as Array<
			keyof typeof result.search
		>) {
	...
	if (overrides.curator) {
		for (const key of Object.keys(overrides.curator) as Array<
			keyof typeof result.curator
		>) {
	...
	if (overrides.crof) {
		for (const key of Object.keys(overrides.crof) as Array<
			keyof typeof result.crof
		>) {
	...
	if (overrides.exa) {
		for (const key of Object.keys(overrides.exa) as Array<
			keyof typeof result.exa
		>) {
	...
	if (overrides.gemini) {
		for (const key of Object.keys(overrides.gemini) as Array<
			keyof typeof result.gemini
		>) {
...
export function getCrofaAiKey(): string | null {
	return (
		normalizeApiKey(process.env["CROFAI_API_KEY"]) ??
		normalizeApiKey(loadConfig().crofAiKey)
	);
}
