...
function resolveBackendParams(params: BackendParams): Required<BackendParams> {
	return {
		...
		model_path:
			params.model_path ?? process.env["FASTEDIT_MODEL_PATH"] ?? DEFAULT_MODEL,
		// biome-ignore lint/complexity/useLiteralKeys: noPropertyAccessFromIndexSignature requires bracket notation
 ...
 */
function pushBackendArgs(
	args: string[],
	resolved: Required<BackendParams>,
): void {
...
export function register(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			"Use fast_diff to review changes made by fast_edit before committing.",
		],
		parameters: Type.Object({
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			replace: Type.Optional(
				Type.String({ description: "Symbol name to replace" }),
			),
			after: Type.Optional(
			...
			snippet: Type.String({
				description:
					"Code snippet to apply. Use #... or //... for lines to preserve.",
			}),
			...
			api_base: Type.Optional(
				Type.String({ description: "vLLM API base URL" }),
			),
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			symbol: Type.String({
				description:
					"Symbol name to delete (e.g. 'my_func' or 'MyClass.method')",
			}),
		}),
		async execute(_toolCallId, params, signal) {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal) {
			return execFastedit(
				["rename", params.file, params.old_name, params.new_name],
				signal,
			);
		},
	});
