...
import { hasBinary, loadConfig } from "../config/config.js";
	...
	try {
		...
		if (result && "exhausted" in result) {
			return {
				...
				results: [],
				error:
					"Exa monthly free tier exhausted (1,000 requests). Resets next month. Upgrade at exa.ai/pricing",
			};
		}
...
function extractQueryList(params: {
	query?: string;
	queries?: string[];
}): string[] {
