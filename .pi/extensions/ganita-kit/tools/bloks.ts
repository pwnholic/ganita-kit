...
export function register(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			names: Type.String({
				description:
					"Package name(s), space-separated (e.g. 'react react-dom')",
			}),
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			symbol: Type.Optional(
				Type.String({
					description:
						"Symbol name (e.g., 'useState'). Omit for deck overview.",
				}),
			),
			...
			all: Type.Optional(
				Type.Boolean({
					description:
						"Show all APIs including internal (default: public only)",
				}),
			),
		}),
		async execute(_toolCallId, params, signal) {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			library: Type.Optional(
				Type.String({ description: "Filter to a specific library" }),
			),
			kind: Type.Optional(StringEnum(["api", "doc", "example"])),
			path: Type.Optional(
				Type.String({ description: "Filter by file path substring" }),
			),
			limit: Type.Optional(
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			budget: Type.Optional(
				Type.Number({ description: "Max output lines. Default: 200" }),
			),
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			tags: Type.Optional(
				Type.String({ description: "Tags (comma-separated)" }),
			),
			from: Type.Optional(
				Type.String({ description: "Import body from a file" }),
			),
		}),
		async execute(_toolCallId, params, signal) {
	...
	pi.registerTool({
		...
		label: "Bloks Report",
		description:
			"Report an error in a context blok. Helps improve future context generation.",
	...
	pi.registerTool({
		...
		description:
			"Per-card feedback. Acknowledge cards as useful (ack) or not useful (nack).",
	...
	pi.registerTool({
		...
		label: "Bloks Nack",
		description:
			"Mark cards as not useful. Supports per-card IDs or bulk session nack.",
	...
	pi.registerTool({
		...
		label: "Bloks Stats",
		description:
			"Show card effectiveness stats. See which cards are most useful.",
	...
	pi.registerTool({
		...
		label: "Bloks List",
		description:
			"List all indexed libraries with names, versions, and index status.",
