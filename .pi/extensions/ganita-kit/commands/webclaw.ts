...
export function register(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			url: Type.Optional(
				Type.String({ description: "URL to scrape (web page or PDF)" }),
			),
			file: Type.Optional(
				Type.String({
			...
			format: Type.Optional(
				StringEnum(["llm", "markdown", "text", "json", "html"]),
			),
			...
			exclude_selectors: Type.Optional(
				Type.String({
					description:
						'CSS selectors to exclude (comma-separated, e.g. "nav,.sidebar")',
				}),
			),
			metadata: Type.Optional(
				Type.Boolean({ description: "Include metadata in output" }),
			),
			timeout: Type.Optional(
				Type.Number({ description: "Request timeout in seconds. Default: 30" }),
			),
			proxy: Type.Optional(
				Type.String({ description: "Proxy URL (http:// or socks5://)" }),
			),
			...
			browser: Type.Optional(
				StringEnum(["chrome", "firefox", "safari-ios", "random"]),
			),
			...
			verbose: Type.Optional(
				Type.Boolean({ description: "Enable verbose logging" }),
			),
			cloud: Type.Optional(
				Type.Boolean({
					description:
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			urls: Type.Optional(
				Type.Array(Type.String(), { description: "URLs to scrape" }),
			),
			urls_file: Type.Optional(
				Type.String({ description: "File with URLs (one per line)" }),
			),
			format: Type.Optional(
				StringEnum(["llm", "markdown", "text", "json", "html"]),
			),
			...
			cookie_file: Type.Optional(
				Type.String({ description: "JSON cookie file" }),
			),
			verbose: Type.Optional(
				Type.Boolean({ description: "Enable verbose logging" }),
			),
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			depth: Type.Optional(
				Type.Number({ description: "Maximum crawl depth. Default: 1" }),
			),
	...
	pi.registerTool({
		...
		promptGuidelines: [
			"Use webclaw_vertical for site-specific structured extraction (e.g., Amazon products, Hacker News posts).",
			"Run webclaw_extractors first to see available vertical extractors.",
		],
		parameters: Type.Object({
			extractor: Type.String({
				description:
					"Vertical extractor name (e.g. reddit, github_repo, amazon_product)",
			}),
			url: Type.String({ description: "URL to extract from" }),
		}),
		async execute(_toolCallId, params, signal) {
			return execWebclaw(
				["vertical", params.extractor, params.url, "-f", "json"],
				signal,
			);
		},
	});
	...
	pi.registerTool({
		...
		async execute(_toolCallId, _params, signal) {
			return execWebclaw(["extractors", "-f", "json"], signal);
		},
	});
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal) {
			...
			return execWebclaw(
				[params.url, "--summarize", String(sentences)],
				signal,
			);
		},
	});
	...
	pi.registerTool({
		...
		promptGuidelines: [
			"Use webclaw_diff to track changes on a web page over time.",
		],
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, signal) {
			return execWebclaw(
				[params.url, "--diff-with", params.snapshot, "-f", "json"],
				signal,
			);
		},
	});
	...
	pi.registerTool({
		...
		promptGuidelines: [
			"Use webclaw_watch to monitor a page for content changes.",
		],
		parameters: Type.Object({
