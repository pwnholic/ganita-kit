...
function getDefaultTimeout(): number {
    return loadConfig().cli.tldrTimeoutMs ?? 60_000;
}
function getProjectTimeout(): number {
    return loadConfig().cli.tldrProjectTimeoutMs ?? 120_000;
}
...
export function register(pi: ExtensionAPI): void {
    ...
    function execTldr(
        ...
        const pi = piRef;
        if (!pi) throw new Error("register() must be called before executing tldr tools");
        return execCli(pi, "tldr", args, signal, timeout ?? getDefaultTimeout());
    }
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            staged: Type.Optional(
                Type.Boolean({
                    description: "Only consider staged files (pre-commit workflow). Default: false",
                }),
            ),
            ...
            base: Type.Optional(
                Type.String({
                    description: "Git base branch for diff (e.g., 'origin/main' for PR workflow)",
                }),
            ),
        }),
        async execute(_id, params, signal) {
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            collapse_packages: Type.Optional(
                Type.Boolean({
                    description: "Collapse files into package-level nodes. Default: false",
                }),
            ),
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            no_callgraph: Type.Optional(
                Type.Boolean({
                    description: "Skip call graph enrichment (much faster). Default: false",
                }),
            ),
        }),
        async execute(_id, params, signal) {
    ...
    pi.registerTool({
        ...
        async execute(_id, params, signal) {
            ...
            return execTldr(args, signal, getProjectTimeout());
        },
    });
    ...
    pi.registerTool({
        ...
        async execute(_id, params, signal) {
            return execTldr(["verify", params.path, "-f", "json"], signal, getProjectTimeout());
        },
    });
    ...
    pi.registerTool({
        ...
        async execute(_id, params, signal) {
            return execTldr(["warm", params.path, "-f", "json"], signal, getProjectTimeout());
        },
    });
